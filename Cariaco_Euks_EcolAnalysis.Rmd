...
```{r}
# save.image("../../DatasetBackupv2/CariacoEuks_postanalysis_vars_upto_tree.RData")
```
...
### Prepare for bubble plots
Average across sample replicates
```{r}
...
#otu_table_mean_ra <- as_tibble(otu_table_mean_ra, rownames = "SampleID")
...
```{r}
ps_ra_mean <- phyloseq(otu_table(otu_table_mean_ra, taxa_are_rows = TRUE), TAX)
```
...
tax_glom(ps_ra_mean, "Division")
```



Link taxonomy to mean RA table- COME BACK HERE AND USE GLOMMED DATA TO SEE PHYLA BUBBLE PLOTS ETC
```{r}
otu_table_mean_ra <- left_join(otu_table_mean_ra, as_tibble(taxonomy, rownames = "SampleID"))
otu_table_mean_ra
```
...
Pivot longer
```{r}
otu_table_mean_ra <- pivot_longer(otu_table_mean_ra, cols = unique(metadata$Replicate), names_to = "Replicate")
otu_table_mean_ra
```
...
```{r}
#C212A
p1 = ggplot(otu_table_mean_ra,aes (x = Depth, y = fct_rev(as_factor(Taxonomy2))))+ # the fancy stuff around y (Taxonomy2) just helps to present it in reverse order in the plot (from top to btm alphabetically)
