...
author: "Liz Suter"
date: "Started 5/26/2020; Completed summer 2021; Revisions added Oct-Nov 2021"
...
Next, trim the sppfit vegan object to just include those species with r2 value greater than 0.60
...
# Oct-Nov 2021 Revisions
In response to reviews, re-run some analyses

Make all networks same size so that they are directly comparable-

### Prepare for whole network analysis
Make subsets that will allow for controlling for network size later on
- top 100 euks, bac, arch (which will be combined into a 300-member dataset)
- top 150 bac and arch (which will be combined into a 300-member dataset of Proks only)
```{r}
...
### Eukaryotic parasite-only and nonparasite-only networks
...
getStability(se_3domains_top100)
```


### 3 Domains, Only nonparasite eukaryotes
...
#Run Spieceasi on 3 domain dataset with top 100 from each domain
pargs <- list(seed=10010)
se_3domains_top100_nonparasites <- spiec.easi(list(ps_bac_pruned_3domains_top100, ps_arch_pruned_3domains_top100, ps_euk_nonparasites_pruned_3domains_top100), method='glasso', lambda.min.ratio=1e-2, nlambda=100, pulsar.params=pargs)
getStability(se_3domains_top100_nonparasites)
```

### 3 Domains, Only parasite eukaryotes
Remove samples from the phyloseq objects that are not in all 3 datasets. Reorder samples so they are in same order in all 3 objects
```{r}
bac_arch_common <- intersect(sample_names(ps_bac_ra_pruned), sample_names(ps_arch_ra_pruned))
all_common <- intersect(bac_arch_common, sample_names(ps_euk_ra_pruned))
...
### Two-Domain dataset
Prkaryotes only
150 from each domain, to keep size of network consistent
...
### Full dataset, with no parasites
First extract ASV names from original phyloseq objects in order to retain for network nodes. Make sure these are in same order in which they went in SpiecEasi
```{r}
...
# Extract weighted adjacency
se.cor  <- cov2cor(as.matrix(getOptCov(se_3domains_top100_nonparasites)))
weighted.adj.mat <- se.cor*getRefit(se_3domains_top100_nonparasites)

#Convert to graph objects
grph.unweighted.top300.nonparasites <- adj2igraph(adj.mat)
grph.top300.nonparasites <- adj2igraph(weighted.adj.mat)


# Put back in species names
V(grph.top300.nonparasites)$name <- rownames(top300_df_nonparasites)


# Make size of nodes proportional to degree (number of connections)
V(grph.top300.nonparasites)$size <- (degree(grph.top300.nonparasites) + 1) # the +1 avoids size zero vertices

# Change width of edges to be proportional to their weights
E(grph.top300.nonparasites)$width <- abs(E(grph.top300.nonparasites)$weight)*10

# Scale node sizes to be smaller
V(grph.top300.nonparasites)$size <- V(grph.top300.nonparasites)$size/2

# Join taxonomy data of each node
# Convert graph to datafram
grph.top300.nonparasites_df <- igraph::as_data_frame(grph.top300.nonparasites, 'both')
...
arch_top100_tax_table <- as.data.frame(tax_table(ps_arch_pruned_3domains_top100)) %>%
  mutate(name = rownames(tax_table(ps_arch_pruned_3domains_top100)))
euk_top100_tax_table <- as.data.frame(tax_table(ps_euk_nonparasites_pruned_3domains_top100)) %>%
  mutate(name = rownames(tax_table(ps_euk_nonparasites_pruned_3domains_top100)))

#link graph data frame to formatted taxonomy tables
bac_temp <- left_join(grph.top300.nonparasites_df$vertices[1:ntaxa(ps_bac_pruned_3domains_top100),],
bac_top100_tax_table, by = "name")
# delete columns that don't match other tax tables
bac_temp <- select(bac_temp, -"taxonomy-9", -"Refined taxonomy")

arch_temp <- left_join(grph.top300.nonparasites_df$vertices[ntaxa(ps_arch_pruned_3domains_top100)+1:ntaxa(ps_arch_pruned_3domains_top100),],arch_top100_tax_table, by = "name")

euk_temp <- left_join(grph.top300.nonparasites_df$vertices[ntaxa(ps_euk_nonparasites_pruned_3domains_top100)+ntaxa(ps_euk_nonparasites_pruned_3domains_top100)+1:ntaxa(ps_euk_nonparasites_pruned_3domains_top100),], euk_top100_tax_table, by = "name")
...
# remake into graph
grph.top300.nonparasites <- graph_from_data_frame(grph.top300.nonparasites_df$edges,
                                   directed = F,
                                   vertices = all_temp)
...
dtype = c("red", "green", "blue", "grey")
# Make color vector
domain_color_top300_nonparasites <- dtype[as.numeric(as.factor(V(grph.top300.nonparasites)$"taxonomy-1"))]


# Plot
plot(grph.top300.nonparasites,
     vertex.label=NA,
     layout=layout_with_graphopt(grph.top300.nonparasites),
     vertex.color=domain_color_top300_nonparasites)
title("SpiecEasi Network: All domains, no parasites")
legend("topright",bty = "n",
       legend=c("Archaea", "Bacteria", "Eukarya", "No blast hit"),
       fill=c("red","green","blue", "grey"), border=NA)
...
# Save plot
setEPS()
postscript(file = "Figures/3domains_nonparasites_top300_spieceasi_network.eps", width = 5.5, height = 5)
plot(grph.top300.nonparasites,
     vertex.label=NA,
     layout=layout_with_graphopt(grph.top300.nonparasites),
     vertex.color=domain_color_top300_nonparasites)
...
### Full dataset, with only parasites from euks
First extract ASV names from original phyloseq objects in order to retain for network nodes. Make sure these are in same order in which they went in SpiecEasi
```{r}
rm(se_3domains_top100_nonparasites)
load("EnvironmentBackups/CariacoEuks_revisions2_additionalvars_6.RData")

top300_df_parasites <- bind_rows(data.frame(otu_table(ps_bac_top100)), data.frame(otu_table(ps_arch_top100)), data.frame(otu_table(ps_euk_parasites_top100)))
top300_df_parasites
```
...
### Save and re-load
```{r}
# remove some of the large spieceasi objects. Have used these to create downstream variables at this point
rm(se, se_3domains_top100_parasites, se.2domains, se.anoxic, se.cor, se.euxinic,se.oxycline)
# save
save.image("EnvironmentBackups/CariacoEuks_postanalysis_vars_upto_Oct2021_newNetworks.RData")
```
...
### Summary network figures for manuscript
...
## Summary network figure with euks color-coded
Reviewer wants to see important groups in Euks labelled

First remake the color vector to include additional colors for Syndiniales and Polycystinea
```{r}
# Top 300, all depths, positive network:
V(grph.top300.pos)$"taxonomy-1"[which(V(grph.top300.pos)$"taxonomy-4" == "Syndiniales")] <- "Syndiniales"
V(grph.top300.pos)$"taxonomy-1"[which(V(grph.top300.pos)$"taxonomy-4" == "Polycystinea")] <- "Polycystinea"
dtype = c("red", "green", "blue", "grey", "purple", "orange") # include "grey" only if there is a node with "No blast hit"
domain_color_top300_pos_extended <- dtype[as.numeric(as.factor(V(grph.top300.pos)$"taxonomy-1"))]

# Top 300, all depths, negative
V(grph.top300.neg)$"taxonomy-1"[which(V(grph.top300.neg)$"taxonomy-4" == "Syndiniales")] <- "Syndiniales"
V(grph.top300.neg)$"taxonomy-1"[which(V(grph.top300.neg)$"taxonomy-4" == "Polycystinea")] <- "Polycystinea"
dtype = c("red", "green", "blue", "grey", "purple", "orange") # include "grey" only if there is a node with "No blast hit"
domain_color_top300_neg_extended <- dtype[as.numeric(as.factor(V(grph.top300.neg)$"taxonomy-1"))]

# Top 300, oxycline, positive
V(grph.top300.oxycline.pos)$"taxonomy-1"[which(V(grph.top300.oxycline.pos)$"taxonomy-4" == "Syndiniales")] <- "Syndiniales"
V(grph.top300.oxycline.pos)$"taxonomy-1"[which(V(grph.top300.oxycline.pos)$"taxonomy-4" == "Polycystinea")] <- "Polycystinea"
dtype = c("red", "green", "blue", "purple", "orange") # include "grey" only if there is a node with "No blast hit"
domain_color_top300_oxy_pos_extended <- dtype[as.numeric(as.factor(V(grph.top300.oxycline.pos)$"taxonomy-1"))]

# Top 300, oxycline, negative
V(grph.top300.oxycline.neg)$"taxonomy-1"[which(V(grph.top300.oxycline.neg)$"taxonomy-4" == "Syndiniales")] <- "Syndiniales"
V(grph.top300.oxycline.neg)$"taxonomy-1"[which(V(grph.top300.oxycline.neg)$"taxonomy-4" == "Polycystinea")] <- "Polycystinea"
dtype = c("red", "green", "blue", "purple", "orange")
domain_color_top300_oxy_neg_extended <- dtype[as.numeric(as.factor(V(grph.top300.oxycline.neg)$"taxonomy-1"))]

# Top 300, anoxic, positive
V(grph.top300.anoxic.pos)$"taxonomy-1"[which(V(grph.top300.anoxic.pos)$"taxonomy-4" == "Syndiniales")] <- "Syndiniales"
V(grph.top300.anoxic.pos)$"taxonomy-1"[which(V(grph.top300.anoxic.pos)$"taxonomy-4" == "Polycystinea")] <- "Polycystinea"
dtype = c("red", "green", "blue", "grey", "purple", "orange") # include "grey" only if there is a node with "No blast hit"
domain_color_top300_anoxic_pos_extended <- dtype[as.numeric(as.factor(V(grph.top300.anoxic.pos)$"taxonomy-1"))]

# Top 300, anoxic, negative
V(grph.top300.anoxic.neg)$"taxonomy-1"[which(V(grph.top300.anoxic.neg)$"taxonomy-4" == "Syndiniales")] <- "Syndiniales"
V(grph.top300.anoxic.neg)$"taxonomy-1"[which(V(grph.top300.anoxic.neg)$"taxonomy-4" == "Polycystinea")] <- "Polycystinea"
dtype = c("red", "green", "blue", "purple", "orange") # include "grey" only if there is a node with "No blast hit"
domain_color_top300_anoxic_neg_extended <- dtype[as.numeric(as.factor(V(grph.top300.anoxic.neg)$"taxonomy-1"))]

# Skip 2 domain no networks (no euks!)

# Top 300, nonparasites, positive (should not have Syndiniales)
V(grph.top300.nonparasites.pos)$"taxonomy-1"[which(V(grph.top300.nonparasites.pos)$"taxonomy-4" == "Syndiniales")] <- "Syndiniales"
V(grph.top300.nonparasites.pos)$"taxonomy-1"[which(V(grph.top300.nonparasites.pos)$"taxonomy-4" == "Polycystinea")] <- "Polycystinea"
dtype = c("red", "green", "blue", "grey", "purple") # remove orange for Syndiniales
domain_color_top300_nonparasites_pos_extended <- dtype[as.numeric(as.factor(V(grph.top300.nonparasites.pos)$"taxonomy-1"))]

# Top 300, nonparasites, negative
V(grph.top300.nonparasites.neg)$"taxonomy-1"[which(V(grph.top300.nonparasites.neg)$"taxonomy-4" == "Syndiniales")] <- "Syndiniales"
V(grph.top300.nonparasites.neg)$"taxonomy-1"[which(V(grph.top300.nonparasites.neg)$"taxonomy-4" == "Polycystinea")] <- "Polycystinea"
dtype = c("red", "green", "blue", "grey", "purple") # remove orange for Syndiniales
domain_color_top300_nonparasites_neg_extended <- dtype[as.numeric(as.factor(V(grph.top300.nonparasites.neg)$"taxonomy-1"))]

# Top 300, parasites, positive (should not have Polycystinea)
V(grph.top300.parasites.pos)$"taxonomy-1"[which(V(grph.top300.parasites.pos)$"taxonomy-4" == "Syndiniales")] <- "Syndiniales"
V(grph.top300.parasites.pos)$"taxonomy-1"[which(V(grph.top300.parasites.pos)$"taxonomy-4" == "Polycystinea")] <- "Polycystinea"
dtype = c("red", "green", "blue", "grey", "orange") # remove orange for Polycystinea
domain_color_top300_parasites_pos_extended <- dtype[as.numeric(as.factor(V(grph.top300.parasites.pos)$"taxonomy-1"))]

# Top 300, parasites, negative
V(grph.top300.parasites.neg)$"taxonomy-1"[which(V(grph.top300.parasites.neg)$"taxonomy-4" == "Syndiniales")] <- "Syndiniales"
V(grph.top300.parasites.neg)$"taxonomy-1"[which(V(grph.top300.parasites.neg)$"taxonomy-4" == "Polycystinea")] <- "Polycystinea"
dtype = c("red", "green", "blue", "grey", "orange") # remove orange for Polycystinea
domain_color_top300_parasites_neg_extended <- dtype[as.numeric(as.factor(V(grph.top300.parasites.neg)$"taxonomy-1"))]
```


Replot, first- All depths, oxycline, anoxic networks:
```{r}
# Set up in panels
op <- par(oma=c(2,.5,.5,0),# Room for the titles and legend
  mfrow=c(3,2),
...
plot(grph.top300.pos,
     vertex.label=NA,
     layout=layout_with_graphopt(grph.top300.pos),
     vertex.color=domain_color_top300_pos_extended)
mtext ("Positive", side = 3, outer = TRUE, line = -1, adj = 0.22, cex = .8)
mtext("All depths", side=2, cex = .8, line = 1.5)
# Panel 2- All depths, negative network
plot(grph.top300.neg,
     vertex.label=NA,
     layout=layout_with_graphopt(grph.top300.neg),
     vertex.color=domain_color_top300_neg_extended)
mtext ("Negative", side = 3, outer = TRUE, line = -1, adj = .8, cex = .8)
# Panel 3- Oxycline, positive network
plot(grph.top300.oxycline.pos,
     vertex.label=NA,
     layout=layout_with_graphopt(grph.top300.oxycline.pos),
     vertex.color=domain_color_top300_oxy_pos_extended)
mtext("Oxycline", side=2, cex = .8, line = 1.5)
# Panel 4- Oxycline, negative network
plot(grph.top300.oxycline.neg,
     vertex.label=NA,
     layout=layout_with_graphopt(grph.top300.oxycline.neg),
     vertex.color=domain_color_top300_oxy_neg_extended)
# Panel 5- Anoxic, positive network
plot(grph.top300.anoxic.pos,
     vertex.label=NA,
     layout=layout_with_graphopt(grph.top300.anoxic.pos),
     vertex.color=domain_color_top300_anoxic_pos_extended)
mtext("Anoxic", side=2, cex = .8, line = 1.5)
# Panel 6- Anoxic, negative network
plot(grph.top300.anoxic.neg,
     vertex.label=NA,
     layout=layout_with_graphopt(grph.top300.anoxic.neg),
     vertex.color=domain_color_top300_anoxic_neg_extended)
# Add legend
par(op) # Leave the last plot
op <- par(usr=c(0,1,0,1), # Reset the coordinates
          xpd=NA)         # Allow plotting outside the plot region
legend(.01,-0.04, c("Archaea","Bacteria", "Syndiniales", "Polycystinea","Other Eukarya", "No Blast Hit"), col=c("red", "green", "orange","purple", "blue","grey"), pch = c(16), box.col=NA, cex = .6, horiz = T, x.intersp = c(0.3))


# Save figure
# Set up EPS and make plot
setEPS(width = 6, height = 7)
postscript("Figures/Networks_pos_neg_all_oxycline_anox_Dec21_Synd_Polycys.eps")
# Set up in panels
op <- par(oma=c(2,.5,.5,0),# Room for the titles and legend
  mfrow=c(3,2),
  mai=c(.15,.3,.15,.1))
...
plot(grph.top300.nonparasites.pos,
     vertex.label=NA,
     layout=layout_with_graphopt(grph.top300.nonparasites.pos),
     vertex.color=domain_color_top300_nonparasites_pos_extended)
mtext("Parasites Removed", side=2, cex = .8, line = 1.5)
# Panel 4- Parasites removed, negative network
plot(grph.top300.nonparasites.neg,
     vertex.label=NA,
     layout=layout_with_graphopt(grph.top300.nonparasites.neg),
     vertex.color=domain_color_top300_nonparasites_neg_extended)
# Panel 5- Nonparasites removed, positive network
plot(grph.top300.parasites.pos,
     vertex.label=NA,
     layout=layout_with_graphopt(grph.top300.parasites.pos),
     vertex.color=domain_color_top300_parasites_pos_extended)
mtext("Nonparasites Removed", side=2, cex = .8, line = 1.5)
# Panel 6- Nonparasites removed, negative network
plot(grph.top300.parasites.neg,
     vertex.label=NA,
     layout=layout_with_graphopt(grph.top300.parasites.neg),
     vertex.color=domain_color_top300_parasites_neg_extended)
# Add legend
par(op) # Leave the last plot
op <- par(usr=c(0,1,0,1), # Reset the coordinates
          xpd=NA)         # Allow plotting outside the plot region
...
# Set up EPS and make plot
setEPS(width = 6, height = 7)
postscript("Figures/Networks_pos_neg_noeuks_noparasites_nononparasites_Synd_Polycyst_Dec21.eps")
# Set up in panels
op <- par(oma=c(2,.5,.5,0),# Room for the titles and legend
  mfrow=c(3,2),
  mai=c(.15,.3,.15,.1))
...
### What are the interestined eukaryotic groups connected to?
```{r}
# Remake dataframe objects from full network graph objects
grph.pos_df <- igraph::as_data_frame(grph.top300.pos, 'both')
grph.pos_df_vert <- grph.pos_df$vertices
grph.neg_df <- igraph::as_data_frame(grph.top300.neg, 'both')
grph.neg_df_vert <- grph.neg_df$vertices
...
ggsave("figures/betweenness_vs_degree_v2.eps",p2, width = 10, height = 6.5, units = c("in"))
...
### How many edges does each group have as an aggregate
```{r}
# remake `all_temp` matrix with taxonomy from the top 300 tax table
bac_top100_tax_table <- as.data.frame(tax_table(ps_bac_pruned_3domains_top100)) %>%
  mutate(name = rownames(tax_table(ps_bac_pruned_3domains_top100)))
arch_top100_tax_table <- as.data.frame(tax_table(ps_arch_pruned_3domains_top100)) %>%
  mutate(name = rownames(tax_table(ps_arch_pruned_3domains_top100)))
...
## Report some mean abundances
To include in main text
```{r}
mean_division_ra <- left_join(division_df, as_tibble(taxonomy, rownames = "ASV"), by = "ASV") %>%
  select(-ASV, -Class, -Order, -Family, -Genus, -Species) 

means <- mean_division_ra %>%
  select(-Kingdom,-Supergroup, -Division) %>%
  rowwise() %>%
  summarise('Mean RA' = mean(c_across(everything())))

sterr <- mean_division_ra %>%
  select(-Kingdom,-Supergroup, -Division) %>%
  rowwise() %>%
  summarise('SE' = sd(c_across(everything()))/sqrt(2))

mean_division_ra$'Mean RA' <- means$'Mean RA'
mean_division_ra$'SE' <- sterr$'SE'
