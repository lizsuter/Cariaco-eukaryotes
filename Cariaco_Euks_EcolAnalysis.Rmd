...
metadata_ordinations <- rename(metadata_ordinations, PartS = ParticulateS, MicroAbun = "MicroAbun(x10^8 L^-1)",  FlagAbun = "FlagAbun(x10^5 L-1)", VLPAbun = "VLP(x10^8 L-1)")
...
pca_envfit_chemo <- envfit(lograt_pca~Chemoautotrophy, metadata_ordinations, permutations = 1000)
...
pca_sppfit_trim<-select.envfit(pca_sppfit, 0.65) 
```
...
with(metadata_ordinations, plot(scores(lograt_pca, display = "sites"), col = colvec[OxCond], pch = shapevec[SizeFraction], cex = 1.5, cex.lab = .8, cex.axis = .8, xlab = "", ylab = paste0('PC2 ', round(lograt_variances[2,2]*100,2),'%'), xaxt='n'))
...
with(metadata_ordinations, plot(scores(lograt_pca, display = "sites"), col = colvec[OxCond], pch = shapevec[SizeFraction], cex = 1.5, cex.lab = .8, cex.axis = .8, xlab = "", ylab = "", xaxt='n', yaxt='n'))
...
with(metadata_ordinations, plot(scores(lograt_pca, display = "sites"), col = colvec[OxCond], pch = shapevec[SizeFraction], cex = 1.5, cex.lab = .8, cex.axis = .8, xlab = paste0('PC1 ', round(lograt_variances[1,2]*100,2),'%'), ylab = paste0('PC2 ', round(lograt_variances[2,2]*100,2),'%')))
...
with(metadata_ordinations, plot(scores(lograt_pca, display = "sites"), col = colvec[OxCond], pch = shapevec[SizeFraction], cex = 1.5, cex.lab = .8, cex.axis = .8, xlab = paste0('PC1 ', round(lograt_variances[1,2]*100,2),'%'), ylab = "", yaxt='n'))
...
# only keep species with p-val < 0.001 and r2 value >0.65
fit_spp <- pca_sppfit %>% 
...
```{r}
# Set up in panels
op <- par(oma=c(0,.5,.5,0),# Room for the titles and legend
  mfrow=c(3,2),
  mai=c(.3,.3,.3,.3))
...
## STOPPED HERE. CONTINUE PLAYING WITH PLOTS BELOW, highliting specific groups (Synd, Spumellaride, Alphas, Deferribac, etc). COLLECT OVERALL NETWORK PARAMETERS AFTER REMOVING EUKS TO SEE HOW NETOWORKS CHANGE
- report blast/ new SILVA results for 
  - "No blast hit" denovo129797
  - "AEGAN-245" from a metagenome denovo351910
- Determine total number of edges connected to euks vs bacteria
...
- Highest betweenness: 2 Defferibacreres/ MG-A OTUs (denovo252731 and denovo249977) and a Gammaproteobacteria. Oceanospiralles/ OM182 clade OTU: denovo279114
- Highest degrees: Several Alphaproteobacteria/SAR11 OTUs, a Chloroflexi/ Anaerolineaceae OTU (denovo317108) and Proteobacteria/ AEGEAN-245 OTU (denovo351910- Blast this one see if there's better taxonomy available now)
- Nodes with mixed high degree/ high betweenness: 2 Thaumarchaeota Group C3 OTUs, an Actinobacteria/ Acidimicrobiales/ Sva0966 marine group OTU, and a SAR11 (Surface_1 clade) OTU
- Low betweenness/ high degree
  - Spirochaetes/ MSBL8
  - Alphaproteobacteria/ OCS116
  - Euryacrchaeota/ Halobacteria/ Marine Hydrothermal Vent Group 
  - Thamarchaeota/ Group C3
  - Candidate division WS3
  - Nitrospinaceae
  - Candidate division OP3
  - Nitrospira OPB95
- Eukaryotes?
  - Rhizaria/ Radiolaria/ Polycystinea/ Spumellarida and Alveolota/ Dinoflagellate/ Syndiniales or Dinphyceae are scattered throughout plot
  - Ciliates present but less so. Notably Cariacotrichea is present
...
**Summary**
- High betweenness but low clustering coef:
  - Deferibacteres/ Marine Group A
  - Some Syndiniales
  - Gammaproteobacteria: OM182 clade and Xanthomonadales/ Sinobacteraceae clade, and Vibrionaceae clade
  - Actinobacteria/ Acidimicrobiales/ Sva0966 marine group OTU
- High betweenness and high clustering coefficient
  - Cnidaria, Crustaceae, several Syndinales, Planctomycetes OM190, Euryarchaeota MG II, 1 Spumellarida
- Low betweenness, High clusering coefficient
  - Thaumarchaeota MG I, 1 Spumellarida,1 Dinoflagelata/ Dinophyceae, and the No Blast Hit OTU (not even domain assigned- denovo129797- blast this one)
