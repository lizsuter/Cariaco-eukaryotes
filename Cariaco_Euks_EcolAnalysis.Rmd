...
## Plot the PCA with environmental variables
...
```{r}
# Rerun PCA in vegan (prcomp is equivalent to vegan function rda), incorporating the variables I want to plot
lograt_pca <- rda(clr_asv_table_ps_filtered ~ O2+H2S+SizeFraction+OxCond,  metadata_ordinations) 

# plot with ggvegan package
autoplot(lograt_pca) 

# to customize use fortify
pca_fort <- fortify(lograt_pca, display = "sites") %>%
bind_cols(metadata_ordinations)
 
ggplot(pca_fort, aes(x = RDA1, y = RDA2, colour = OxCond, group = SizeFraction)) +
geom_point() 


## STOPPED HERE 7/4. CONTINUE CCUSTOMIZING. USE https://quantpalaeo.wordpress.com/2019/04/11/funky-ordination-plots-with-ggvegan/
+ 
...
layers = "sites", color = factor(OxCond))
...
## STOPPED HERE. CONTINUE PLAYING WITH PLOTS ABOVE, highliting specific groups (Synd, Spumellaride, Alphas, Deferribac, etc). COLLECT OVERALL NETWORK PARAMETERS AFTER REMOVING EUKS TO SEE HOW NETOWORKS CHANGE
