...
Filter out the samples with low sequencing effort. These were previously identified in itags paper
...
### Agglomerate and remove OTUs with low abundance

First calculate relative abdunance
```{r}
...
Agglomerate prokaryotic tables at a higher tax levl than species. Use a tax level that is recognizeable as a notable group. For bacteria, choose "taxonomy-4" (Order; gives enough resolution to pull out SAR406, SAR324, SAR11, Chromatiales, etc, without losing many of the important groups that have "NA" in tax-5 level). For archaea, choose taxonomy-3 (Class- gives enough resolution to differentiate MG I, II, Thermoplasmata, etc. without losing important taxa annotated as "uncultured" at tax-4 level).
```{r}
ps_bac_ra_orders = tax_glom(ps_bac_ra, "taxonomy-4")
ps_arch_ra_classes = tax_glom(ps_arch_ra, "taxonomy-3")
```


And agglomerate the euk table at Order
```{r}
ps_euk_ra_orders = tax_glom(ps_ra, "Order")
```

Save and re-load (above 2 steps took a few minutes)
```{r}
# save.image("EnvironmentBackups/CariacoEuks_postanalysis_vars_upto_glom_of3domains.RData")
```
Re-load
```{r}
load("EnvironmentBackups/CariacoEuks_postanalysis_vars_upto_glom_of3domains.RData")
```


Remove rows of glommed taxa if their sum across all samples doesn't exceed 5% (RA > 0.05) 
```{r}
# Bacteria
x <- taxa_sums(ps_bac_ra_orders)
# keepTaxa <-  base::which(x  > .05)
keepTaxa <-  x>.05 # prune_taxa require a logical not a list of IDs. compare to keepTaxa above to check
ps_bac_ra_pruned <-  prune_taxa(keepTaxa, ps_bac_ra_orders)
ps_bac_ra_pruned

# Archaea
x <- taxa_sums(ps_arch_ra_classes)
# keepTaxa <-  base::which(x  > .05)
keepTaxa <-  x>.05 # prune_taxa require a logical not a list of IDs. compare to keepTaxa above to check
ps_arch_ra_pruned <-  prune_taxa(keepTaxa, ps_arch_ra_classes)
ps_arch_ra_pruned

# Eukaryotes
x <- taxa_sums(ps_euk_ra_orders)
# keepTaxa <-  base::which(x  > .05)
keepTaxa <-  x>.05 # prune_taxa require a logical not a list of IDs. compare to keepTaxa above to check
ps_euk_ra_pruned <-  prune_taxa(keepTaxa, ps_euk_ra_orders)
ps_euk_ra_pruned
```
Trimmed to 75 bacteria OTUs, 7 archaea OTUs, and 28 eukaryotic ASVs (110 total). Proceed with this dataset of the most abundant OTUs for correlations and network analyses...



Make equivalent ps objects that have the same filtered dataset but retain the raw counts instead of the relative abundances (eg. an OTU table)
```{r}
alltaxa_bac <-  taxa_names(ps_bac)
alltaxa_arch <-  taxa_names(ps_arch)
alltaxa_euk <-  taxa_names(ps)

remaintaxa_bac = taxa_names(ps_bac_ra_pruned)
remaintaxa_arch = taxa_names(ps_arch_ra_pruned)
remaintaxa_euk = taxa_names(ps_euk_ra_pruned)

removetaxa_bac = alltaxa_bac[alltaxa_bac %in% remaintaxa_bac]
removetaxa_arch = alltaxa_arch[alltaxa_arch %in% remaintaxa_arch]
removetaxa_euk = alltaxa_euk[alltaxa_euk %in% remaintaxa_euk]


ps_bac_pruned <- prune_taxa(removetaxa_bac,ps_bac)
ps_arch_pruned <- prune_taxa(removetaxa_arch,ps_arch)
ps_euk_pruned <- prune_taxa(removetaxa_euk,ps)
```


### Concatenate all domains into one table

To do this I have to first match the sample names across domain datasets. These currently have "B" for bacteria, A, E etc. Remove this letter from sample names so that "AE2a247B", "AA2a247B", "AB2a247B" all become just "Type" from the metadata sheet [IntNov1FL in this case- for Interface, November, rep 1, free-living]. 
...
Do some manual curating of the filtered OTU table to retain meaningful names
Change row names from "denovoXXX" to meaningful names
```{r}
alldomains_df_full <- cbind(ID = rownames(alldomains_df), alldomains_df)

# start with only first 75 rows, which are bacteria. make one column of meaningful labels
temp1 <- left_join(alldomains_df_full[1:75,], bac_taxonomy, by = c("ID" = "#OTU ID")) 
...
temp2 <- left_join(alldomains_df_full[76:82,], arch_taxonomy, by = c("ID" = "#OTU ID")) 
...
temp3 <- left_join(alldomains_df_full[83:110,], euk_taxonomy, by = c("ID" = "#ASV ID")) 
...
NOTE- input for SparCC should be raw count data (after filtering out low-abundance ASVs). The function does a log-ration transformation to account for compositionality
...
sparcctable_alldomains$Cor[1:5,1:5]
...
## STOP HERE 6/10- next plot p-values and try glasso

### Graphical Lasso
Try the glasso method, as described in the [spieceasi github](https://github.com/zdk123/SpiecEasi) and [BVCN lessons 1.2](https://biovcnet.github.io/_pages/NetworkScience_glasso)
...
# ALL BELOW IS FROM ANOTHER NOTEBOOK. NEEDS MODIFICATION
